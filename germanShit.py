...
def run_predictions(season, game_week):
    sims_to_run = 100000
    df = pd.DataFrame(
        columns=["week", "homeTeam", "awayTeam", "homeWins", "awayWins", "draws", "homeScore", "awayScore"])
    # only use games before the game week we want to predict
    historical = season.loc[season["Wk"] < week]
    ...
    home_avg = historical["HomeScore"].mean()
    away_avg = historical["AwayScore"].mean()
    ...
    for i in to_predict.index:
        ...
        home_team_average_for = historical.loc[season["Home"] == home_team, "HomeScore"].mean()
        home_team_average_against = historical.loc[season["Home"] == home_team, "AwayScore"].mean()

        # average goals scored and goals conceded for away team
        away_team_average_for = historical.loc[season["Away"] == away_team, "AwayScore"].mean()
        away_team_average_against = historical.loc[season["Away"] == away_team, "HomeScore"].mean()
